...
void subBytes(HexWord *A){
    for(unsigned char i = 0; i < 4; i++){
        for(unsigned char j = 0; j < 4; j++){   // Make it better so that it does not uses swapNibbles
            A[i].bytes[j].byte = swapNibbles(sBox[((int)A[i].bytes[j].nibbles.high) * 16 + ((int)A[i].bytes[j].nibbles.low)]);
        }
    }
}
...
void shiftRows(HexWord *inWord){
    unsigned char tempHex1, tempHex2, tempHex3;

    inWord[0].bytes[0].byte = inWord[0].bytes[0].byte;
    inWord[1].bytes[0].byte = inWord[1].bytes[0].byte;
    inWord[2].bytes[0].byte = inWord[2].bytes[0].byte;
    inWord[3].bytes[0].byte = inWord[3].bytes[0].byte;

    tempHex1 = inWord[0].bytes[1].byte;
    inWord[0].bytes[1].byte = (inWord[1].bytes[1].byte);
    inWord[1].bytes[1].byte = (inWord[2].bytes[1].byte);
    inWord[2].bytes[1].byte = (inWord[3].bytes[1].byte);
    inWord[3].bytes[1].byte = (tempHex1);

    tempHex1 = inWord[0].bytes[2].byte;
    tempHex2 = inWord[1].bytes[2].byte;
    inWord[0].bytes[2].byte = (inWord[2].bytes[2].byte);
    inWord[1].bytes[2].byte = (inWord[3].bytes[2].byte);
    inWord[2].bytes[2].byte = (tempHex1);
    ...
    inWord[1].bytes[3].byte = (tempHex1);
    inWord[2].bytes[3].byte = (tempHex2);
    inWord[3].bytes[3].byte = (tempHex3);
}
